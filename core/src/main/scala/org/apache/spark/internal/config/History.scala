...
private[spark] object History {
  ...
  val THREAD_DUMP_COLLECTOR_LOG_CLEANER_ENABLED = ConfigBuilder("spark.history.fs.driverlog" +
    ".cleaner.enabled")
    .version("4.0.0")
    .doc("Specifies whether the History Server should periodically clean up driver logs from " +
      "storage.")
  ...
  val MAX_THREAD_DUMP_COLLECTOR_LOG_AGE_S = ConfigBuilder("spark.history.fs.driverlog.cleaner" +
    ".maxAge")
    .version("4.0.0")
    .doc(s"When ${THREAD_DUMP_COLLECTOR_LOG_CLEANER_ENABLED.key}=true, driver log files older " +
      s"than this will be " +
      s"deleted when the driver log cleaner runs.")
    .fallbackConf(MAX_LOG_AGE_S)

  val THREAD_DUMP_COLLECTOR_LOG_CLEANER_INTERVAL = ConfigBuilder("spark.history.fs.driverlog" +
    ".cleaner.interval")
    .version("4.0.0")
    .doc(s" When ${THREAD_DUMP_COLLECTOR_LOG_CLEANER_ENABLED.key}=true, specifies how often the " +
      s"filesystem " +
      s"driver log cleaner checks for files to delete. Files are only deleted if they are older " +
      s"than ${MAX_DRIVER_LOG_AGE_S.key}.")
    .fallbackConf(CLEANER_INTERVAL_S)
